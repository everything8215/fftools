...
class FF4MapTileset extends ROMToolbox {
  ...
  constructor(gui, map) {
    ...
    this.layer = [new FF4MapLayer(gui, FF4MapLayer.Type.layer1),
    ...
    show() {
      // init the toolbox
      this.addButtons(['Layer 1', 'Layer 2', 'Layer 3', 'Triggers']);
      const self = this;
    ...
    mouseDown(e) {
      ...
      if (this.map.l === 0) {
        this.map.selectTileProperties(x + y * 16)
      };
    }
    ...
    loadMap(m) {
      ...
      if (this.map.isWorld) {
        ...
        this.layer[0].loadLayout({
          ...
          w: 16, h: 8,
          tilePalPath: this.map.layer[0].tilePalPath
        });
